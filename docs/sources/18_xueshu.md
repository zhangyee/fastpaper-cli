...
|---|---|
| `wd` | 搜索词,URL 编码(必需) |
| `pn` | 结果偏移;首页 `0`,翻页 `pn += 10`(实现固定 `pn=0`) |
| `skipStrategy` | 固定传 `0` |

每页固定 10 条。**实现只发一个请求(`pn=0`)、不翻页**:解析该页结果后按 `--offset` 本地跳过、截断到 `-n`;不足 `-n` 就少返回,不会再取下一页。另有 `sort` / `filter` / `slang` 参数(前端代码确认存在,未逐值验证,实现未用)。
...
- 单请求成功率实测约 50%(2026-07-31)。因此实现只发一个请求:翻两页会让整次搜索的成功率降到约 25%,且多出的请求会推高风控评分、拖累后续调用。退避重试同样是反效果(实测 16 请求内升级为持续拦截)。
...
| CLI 参数 | 映射 |
|---|---|
| `-n` / `--offset` | 单页内本地切片。接口每页 10 条而实现只取首页,所以 `-n` 的有效上限约为 10;`--offset` 落在该页之外返回空 |
| `--patents` | **本地过滤**`type == 5`(专利);不带该参数则反过来排除 type=5。接口的 `filter` 参数实测被忽略(2026-07-28 试过 `type:5`/`type=5`/`paper_type:5`/`5`,返回结果完全一致),所以只能在解析时分流。代价:排除专利后该页产出不足 10 条,返回数会少于 `-n`(不再翻页去补) |
