                ...
                cli::SkillAction::Export { agent } => {
                    if let Some(target) = agent {
                        let path = match target {
                            ...
                            cli::AgentTarget::Qwen => ".qwen/skills/fastpaper/SKILL.md",
                        };
                        eprintln!("Install to: {}", path);
                    }
                    print!("{}", SKILL_CONTENT);
                }
                cli::SkillAction::Install { agent } => {
                    let agent = match agent {
                        Some(a) => a,
                        None => {
                            eprintln!("Error: --agent is required for skill install.");
                            eprintln!("Usage: fastpaper skill install --agent <claude|codex|cursor|gemini>");
                            std::process::exit(1);
                        }
                    };
                    ...
                    // 1. Write source file to .agents/skills/fastpaper/SKILL.md
                    let source_path = home_path.join(".agents/skills/fastpaper/SKILL.md");
                    ...
                    std::fs::write(&source_path, SKILL_CONTENT).unwrap_or_else(|e| {
                        eprintln!("Error writing file: {}", e);
                        std::process::exit(1);
                    });
                    eprintln!("Source: {}", source_path.display());

                    // 2. Create symlink at agent-specific path
                    let agent_rel = match agent {
                        cli::AgentTarget::Claude => ".claude/skills/fastpaper/SKILL.md",
                        cli::AgentTarget::Codex => ".codex/skills/fastpaper/SKILL.md",
                        cli::AgentTarget::Cursor => ".cursor/skills/fastpaper/SKILL.md",
                        cli::AgentTarget::Gemini => ".gemini/skills/fastpaper/SKILL.md",
                    ...
                    let link_path = home_path.join(agent_rel);
                    ...
                    std::os::unix::fs::symlink(&source_path, &link_path).unwrap_or_else(|e| {
                        eprintln!("Error creating symlink: {}", e);
                        std::process::exit(1);
                    });
                    eprintln!("Linked: {}", link_path.display());
                }
            }
        }
